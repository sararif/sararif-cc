...
# ── ความกว้างบรรทัด: นับ "ที่ตาเห็น" ไม่ใช่จำนวนตัวอักษร ────────────────
# สระบน/สระล่าง/วรรณยุกต์/การันต์ ซ้อนอยู่บนพยัญชนะ ไม่กินความกว้างบรรทัดเลย
# (ชุดเดียวกับที่ scripts/from_capcut.py ใช้ถ่วงน้ำหนักเวลา)
#
# 🐛 บั๊กที่แก้ 17 ก.ย. 2026 — ของเดิมนับ codepoint ดิบ จึงคิดว่าบรรทัดเต็มเร็วกว่าความจริง
#    แล้วไปตัดกลางคำ ตัวอย่างที่เจอจริงตอนเทียบกับซับที่คนตัดเอง:
#      · "เฟรนช์ฟรายส์"    นับดิบ 12 (เกินเพดาน 11 → ถูกผ่าเป็น "เฟรนช์ฟราย/ส์ก็มี")
#                          กว้างจริง 10 — อยู่บรรทัดเดียวได้สบาย
#      · "อ่างล้างหน้า"    นับดิบ 12 · กว้างจริง 9
#      · "เครื่องทำน้ำอุ่น" นับดิบ 16 · กว้างจริง 11 = พอดีบรรทัด ไม่ต้องซอยเลย
COMBINING = set("ั") | {chr(c) for c in range(0x0E34, 0x0E3B)} | {chr(c) for c in range(0x0E47, 0x0E4F)}


def disp_len(s):
    """ความกว้างที่ตาเห็น — ใช้ตัวนี้ทุกที่ที่เทียบกับเพดานบรรทัด"""
    return sum(1 for ch in s if ch not in COMBINING and not ch.isspace())
...
TRAILING_BAN = {
    ...
    "ทาง", "ในการ", "ของการ",
    # คำเชื่อม
    "และ", "หรือ", "แต่", "ว่า", "ซึ่ง", "เพราะ", "เพราะว่า", "ก็", "จน", "เพื่อ",
    # 🐛 17 ก.ย. 2026 — ถอด "แล้ว" ออกจากทั้ง TRAILING_BAN และ LEADING_BAN
    #    ซับที่คนตัดเองมีทั้งบรรทัดที่ *จบ* ด้วย "แล้ว" ("ของมาแล้ว")
    #    และบรรทัดที่ *ขึ้นต้น* ด้วย "แล้ว" ("แล้วโรยพริกมาเต็ม" · "แล้วโซนนี้")
    #    มันเป็นคำจบประโยค/เปิดประโยค ไม่ใช่คำเชื่อมที่ต้องเกาะ
...
LEADING_BAN = {
    ...
    "นี้", "นั้น", "นี่", "นั่น", "ๆ", "กัน", "เลย", "อยู่", "ไว้", "ด้วย",
...
def group_v2(words, maxchars, gap, breaks=(), scenes=()):
    ...
    for tk, st, en in words:
        ...
        if cur and tk not in ENDERS and not ends_badly() and not cant_lead(tk) and (
            curlen() + disp_len(tk) > maxchars or (st - last) > gap
        ):
            flush()
    ...
    flush()
    return merge_orphans(lines)


def merge_orphans(lines):
    """ยุบบรรทัดเศษ (สั้นกว่า MINLEN) เข้ากับบรรทัดข้างเคียง

    🐛 เพิ่ม 17 ก.ย. 2026 — กฎขอบกล่อง/รอยตัดภาพ "ตัดแน่นอน" ทำให้เกิดบรรทัดคำเดียวโดดได้
    เช่นได้บรรทัด "รอ" · "ครับ" · "ด้วย" ลอยเดี่ยว ซึ่งกะพริบผ่านตาเร็วเกินอ่านทัน
    เทียบกับซับที่คนตัดเอง เขายุบบรรทัดพวกนี้เข้าบรรทัดก่อนหน้าทุกใบ ไม่เคยปล่อยไว้

    เกาะบรรทัด *ก่อนหน้า* เป็นหลัก (เศษมักเป็นหางของประโยคที่แล้ว) ไม่มีก่อนหน้าค่อยเกาะถัดไป
    """
    if len(lines) < 2:
        return lines
    out = []
    for st, en, tx in lines:
        if out and disp_len(tx) < MINLEN:
            pst, pen, ptx = out[-1]
            sep = " " if (ptx[-1:].isascii() and ptx[-1:].isalpha()) or (tx[:1].isascii() and tx[:1].isalpha()) else ""
            out[-1] = (pst, en, ptx + sep + tx)
        else:
            out.append((st, en, tx))
    # บรรทัดแรกสั้นเกิน แต่ตอนนั้นยังไม่มีบรรทัดก่อนหน้าให้เกาะ — เกาะบรรทัดถัดไปแทน
    if len(out) > 1 and disp_len(out[0][2]) < MINLEN:
        (st, _, tx), (_, en2, tx2) = out[0], out[1]
        sep = " " if (tx[-1:].isascii() and tx[-1:].isalpha()) or (tx2[:1].isascii() and tx2[:1].isalpha()) else ""
        out[0:2] = [(st, en2, tx + sep + tx2)]
    return out
