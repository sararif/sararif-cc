...
- แก้คำผิดที่ STT ได้ยินเพี้ยน (คำที่แก้ ใส่เองใน ~/.sararif-cc/fix.txt)
...
# ── คลังคำ ────────────────────────────────────────────────────────────
# pythainlp ไม่รู้จักคำพวกนี้ → ซอยกลางคำ แล้วซับขึ้นคนละบรรทัด
# ในโค้ดเก็บเฉพาะคำที่ครีเอเตอร์ไทยใช้กันทั่วไป
# ชื่อร้าน/ชื่อคน/ชื่อแบรนด์ของแต่ละคน = ใส่เองในไฟล์ ~/.sararif-cc/words.txt
LOANWORDS = {
    "เทรน","เทรนด์","ไลฟ์","คลิป","รีวิว","โฟลเดอร์","ไฟล์","แบต","ครีเอเตอร์",
    "สกรีน","คอนเทนต์","โซโลพรีเนอร์","แอร์ดรอป","ซับ","เอฟเฟกต์","ออนไลน์","โพสต์","เอไอ",
    "พรีเมียม","คู่ค้า","อีคอมเมิร์ซ","ดิจิตอล","แคปชัน",
    # ── เพิ่ม 17 ก.ย. 2026 — คำอาหาร/คาเฟ่ ที่ pythainlp ซอยแหลกจนซับตัดกลางคำ
    #    เจอจากการเทียบซับโปรแกรมกับซับที่คนตัดเอง แล้วดูว่าคนรวมคำไหนกลับ
    #    ตัวอย่าง: "เฟรนช์ฟรายส์" → [เฟรนช์·ฟ·ราย·ส์] · "เทริยากิ" → [เท·ริ·ยา·กิ]
    #    เป็นหมวดที่โดนบ่อยสุดเพราะคลิปรีวิวร้านอาหารพูดคำพวกนี้ตลอด
    "เฟรนช์ฟรายส์","เทริยากิ","ซิกเนเจอร์","บิงซู","ชาบู","ซูชิ","ติ่มซำ","บาริสต้า",
    "ครัวซองต์","อัลมอนด์","แซลมอน","ไอซิ่ง","เคาน์เตอร์","เทมปุระ","วาซาบิ",
    "ยากินิกุ","โอมากาเสะ","ฟิน",
}

USER_DIR = pathlib.Path.home() / ".sararif-cc"
WORDS_FILE = USER_DIR / "words.txt"
FIX_FILE = USER_DIR / "fix.txt"


def _lines(path):
    """อ่านไฟล์ตั้งค่าของผู้ใช้ — บรรทัดว่างและบรรทัดที่ขึ้นต้นด้วย # ข้ามไป"""
    if not path.exists():
        return []
    out = []
...
# คำของผู้ใช้เอง — 1 บรรทัด 1 คำ (ชื่อร้าน ชื่อคน ชื่อสินค้า ที่ไม่อยากให้ถูกตัดกลาง)
USER_WORDS = set(_lines(WORDS_FILE))
LOANWORDS |= USER_WORDS
CUSTOM_DICT = dict_trie(set(thai_words()) | LOANWORDS)

# ── แก้คำที่ถอดเสียงมาผิด ──────────────────────────────────────────────
# apply_fix รองรับความยาวไม่เท่ากัน (สร้าง timing ใหม่ให้ช่วงที่ถูกแทน)
# ชื่อแบรนด์/ชื่อร้านของแต่ละคนไม่เหมือนกัน → ใส่เองในไฟล์ ~/.sararif-cc/fix.txt
#   รูปแบบ 1 บรรทัด:  คำที่ได้ยินผิด = คำที่ถูก
FIX = {}
for ln in _lines(FIX_FILE):
    if "=" not in ln:
        continue
...
def deshatter(toks):
    """เชื่อม "เศษคำ" ที่ตัวตัดคำซอยแหลกกลับเข้ากับคำข้างหน้า

    pythainlp ไม่รู้จักคำยืม จึงซอยเป็นชิ้นที่ไม่ใช่คำ —
      · "เฟรนช์ฟรายส์" → [เฟรนช์·ฟ·ราย·ส์]
      · "เทริยากิ"      → [เท·ริ·ยา·กิ]
    ถ้าปล่อยไว้ จุดตัดบรรทัดจะไปตกกลางคำได้ทุกเมื่อ ซึ่งเป็นข้อที่คนตัดซับเองแก้บ่อยที่สุด

    เจอคำไหนบ่อย ใส่ลงไฟล์ `~/.sararif-cc/words.txt` ได้เลย — ตัวนี้เป็นตาข่ายกันคำที่ยังไม่มีในคลัง
    เงื่อนไข "เศษ": ไม่ใช่คำในคลังไทย · ไม่ใช่อังกฤษ · กว้างไม่เกิน 3 (ไม่นับสระ/วรรณยุกต์)
    """
    known = set(thai_words()) | LOANWORDS
    out = []
    for t in toks:
        w = t.strip()
        frag = (w and w != "ๆ" and w not in known
                and not re.search(r"[A-Za-z0-9]", w)
                and sum(1 for ch in w if ch not in _COMBINING) <= 3)
        if out and frag:
            out[-1] += t
        else:
            out.append(t)
...
_COMBINING = set("ั") | {chr(c) for c in range(0x0E34, 0x0E3B)} | {chr(c) for c in range(0x0E47, 0x0E4F)}


def words_with_time(text, times):
    """tokenize แล้ว map แต่ละคำกลับไป timing ตามตำแหน่งตัวอักษร"""
    toks = deshatter(word_tokenize(text, engine="newmm", keep_whitespace=False,
