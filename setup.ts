...
console.log(`
════════════════════════════════════════════════════════
 โมเดลสำเร็จรูป — เลือกได้ 3 แบบ (ซับ + hook + สัดส่วนจอ ครบชุด)
════════════════════════════════════════════════════════
${Object.entries(STYLES).map(([k, s]) => `
  --style ${k.padEnd(9)} ${s.label}${k === f.style ? "   ← ใช้อยู่ตอนนี้" : ""}
     ${s.desc}
     จอ ${s.ratio} · คลิปเป้า ${s.clipSeconds.min}-${s.clipSeconds.max} วิ · ซับ ${s.maxchars} ตัว/บรรทัด ขนาด ${s.size} · hook ขนาด ${s.hook.size}`).join("\n")}

  ลองสลับดูได้:  bun go.ts myproj --style genz
  ถูกใจแบบไหน ใส่ "style": "genz" ในไฟล์ตั้งค่า จะได้ไม่ต้องพิมพ์ทุกครั้ง
  โปรเจกต์จอไม่ตรงกับโมเดล ระบบจะเตือนให้เอง (ใช้ข้ามได้ ไม่ block)
